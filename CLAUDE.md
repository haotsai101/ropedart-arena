...
- Owns the rope itself, in two entirely different forms depending on `dart == null`, both driven from `_update_persistent_rope()` (called from `_process()`/`_physics_process()`):
  ...
  - **Verification note**: the pivot-placement fix was confirmed by actually measuring the real world-space distance between each joint's two true attachment points every tick (not body centers, not "no errors printed") via a temporary probe, both at rest and under ~90s of hard-bot stress testing — this is what caught both the original clustered-offset bug and the bias-tuning explosion, after "no engine errors" had already failed to catch the first one. **Still not verified visually** — no screenshot access this session, so whether the now-numerically-tight chain actually *reads* as a continuous rope on screen (versus e.g. still showing faint seams between capsule/cylinder segments) is unconfirmed.
  - **Known, disclosed limitation**: the chain is simulated for the dart's *entire* time out (FLYING/ANCHORED/RECALLING alike), including while the tip anchor is being dragged at up to 18–36 units/sec during a fast throw or recall. Measured gap spikes during this (~10–12 units, decaying within a few ticks) never diverged, but how that transient actually looks on screen is unverified. If the user reports jitter specifically during the brief FLYING/RECALLING window, that's the area to revisit first — and re-run the gap-measurement probe technique above rather than re-tuning bias/damping blind.
