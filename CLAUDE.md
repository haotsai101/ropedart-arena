...
- `_render_rope_extended()` also runs a real physics raycast, hand → dart, every frame (every player body excluded, same as `rope_dart.gd`'s own raycast) — most relevant while `ANCHORED`, when the owner (or a retrieving bot) can end up on any side of an obstacle relative to a dart stuck on its far face. If blocked, the rope is truncated at the obstruction instead of drawing straight through solid geometry — real collision data, not hand-rolled corner/edge math.
