...
- Owns the rope itself as a single persistent object (`_rope_segments`, built once in `_setup_dagger_in_hand()`) rather than the dart spawning its own — `_update_persistent_rope()`, called from `_process()`, redraws the same `ROPE_SEGMENTS` cylinders every frame as either a tight coil around the forearm (`_render_rope_coiled()`, anchored at a `lowerarm.r` `BoneAttachment3D`, while `dart == null`) or a straight sagging line out to the dart (`_render_rope_extended()`, reading `dart.plane_y`/`dart.head_mesh` by duck typing, while a dart is out). One object changing shape, not a coil mesh swapped for a separately-spawned tether — reads as "uncoiling" without any literal unspooling animation.
- `_render_rope_extended()` also runs a real physics raycast **per segment** along the rope's actual sampled (sagging) curve, not just once for the straight hand→dart span — a single whole-span raycast can miss an obstruction that only intersects the sag's curved middle. Every player body is excluded (same as `rope_dart.gd`'s own raycast), most relevant while `ANCHORED`, when the owner (or a retrieving bot) can end up on any side of an obstacle relative to a dart stuck on its far face. The first blocked segment collapses every point after it to the obstruction, so the rope visibly stops there instead of drawing through solid geometry — real per-segment collision data, not hand-rolled corner/edge math.
...
- The rope itself is no longer this script's — see `player.gd`'s `_update_persistent_rope()`. The extended line is a plain hand-to-dart segment with a shallow parabolic sag, no obstacle-routing geometry — there's no bend/corner logic to keep it clear of a pillar the owner has walked behind post-anchor (a deliberate simplification; the older corner-routing math kept re-breaking and was removed). Only the dart's own flight is obstacle-aware, via the real physics raycast above.
